...
$_bash       = $db->queryArrayRow("SELECT * FROM bash_approved WHERE bash_razdel = 'index' ORDER BY bash_rating DESC LIMIT ".$start.", ".$num);
...
($p > 1 ? $out .= ' <a href="/byrating/'.($p-1).'">предыдущая</a> ' : $out .= '');

for($i = 0; $i < count($pages); $i++){
$s++;
($pages[$i] != $s ? $out .= ' ... ' : $out .= '');
($pages[$i] == $p ? $out .= ' <span>'.$pages[$i].'</span> ' : $out .= ' <a href="/byrating/'.$pages[$i].'">'.$pages[$i].'</a> ');
$s = $pages[$i];
}

($p < $total && $total > 1 ? $out .= ' <a href="/byrating/'.($p+1).'">следующая</a> ' : $out .= '');
