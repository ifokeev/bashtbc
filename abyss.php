...
$_bash       = $db->queryArrayRow("SELECT * FROM bash_approved AS bash1 JOIN (SELECT ROUND(RAND() * (SELECT MAX(bash_app_id) FROM bash_approved)) AS id) AS bash2 WHERE bash1.bash_app_id >= bash2.id AND bash1.bash_razdel = 'abyss' ORDER BY bash1.bash_app_id ASC LIMIT 50");
...
$_app_today  = $db->numrows("SELECT * FROM bash_approved WHERE bash_date_post LIKE '%".date('Y-m-d')."%' AND bash_razdel = 'index'");
               $db->execQuery("DELETE FROM bash_approved WHERE UNIX_TIMESTAMP(bash_date_post) <= '".(time()-172800)."' AND bash_razdel = 'abyss'");
