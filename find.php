...
@$p         = intval($_GET['p']);
@$_GET['q'] = textfilter($_GET['q']);

if(!empty($_GET['q'])){

$_num  = $db->numrows("SELECT * FROM bash_approved WHERE (bash_text LIKE '%{$_GET['q']}%' OR bash_app_id = '{$_GET['q']}') AND bash_razdel = 'index'");
...
$_bash = $db->queryArrayRow("SELECT * FROM bash_approved WHERE (bash_text LIKE '%{$_GET['q']}%' OR bash_app_id = '{$_GET['q']}') AND bash_razdel = 'index' ORDER BY bash_date_post DESC LIMIT ".$start.", ".$num);
...
($p > 1 ? $out .= ' <a href="/find?p='.($p-1).'&q='.$_GET['q'].'">предыдущая</a> ' : $out .= '');

for($i = 0; $i < count($pages); $i++){
$s++;
($pages[$i] != $s ? $out .= ' ... ' : $out .= '');
($pages[$i] == $p ? $out .= ' <span>'.$pages[$i].'</span> ' : $out .= ' <a href="/find?p='.$pages[$i].'&q='.$_GET['q'].'">'.$pages[$i].'</a> ');
$s = $pages[$i];
}

($p < $total && $total > 1 ? $out .= ' <a href="/find?p='.($p+1).'&q='.$_GET['q'].'">следующая</a> ' : $out .= '');
...
<? include _ROOTPATH.'bash_inc/basn_reklama_box.php'; ?>
       ...
       <div id="search_block">
            	<p>Введите ключевую фразу для поиска или номер цитаты:</p>
                <div id="search_box">
                	<div id="search1">
                    	<div id="search2">
                        	<div id="search3">
                            	<div id="search4">
                                     ...
                                     <div id="box_search">
                                        <div id="search_l"><img src="http://<?=$db_config['IMGserver'];?>/search_l.gif" alt=""/></div>
                                        <div id="search_input"><input name="q"/></div>
                                        <div id="search_r"><img src="http://<?=$db_config['IMGserver'];?>/search_r.gif" alt=""/></div>
                                    </div>
                                    <div id="button"><input type="submit" value="" /></div>
                                    </form>
                                </div>
...
<? if(!empty($_bash)): ?>
           <div id="reklama_box">
                <div class="navigation" style="font-family: tahoma, sans-serif; FONT-SIZE: 11px; margin-bottom:10px; margin-top:10px; padding: 0px 25px;">Страницы:&nbsp;&nbsp;&nbsp;
<?=$out;?>
                </div>
           </div>
