...
$_bash       = $db->queryArrayRow("SELECT * FROM bash_approved AS bash1 JOIN (SELECT ROUND(RAND() * (SELECT MAX(bash_app_id) FROM bash_approved)) AS id) AS bash2 WHERE bash1.bash_app_id >= bash2.id AND bash1.bash_razdel = 'index' ORDER BY bash1.bash_app_id ASC LIMIT 50");
